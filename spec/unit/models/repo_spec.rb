...
RSpec.describe Pivot::GitHub::Repo do
  describe '.from_octokit_repo' do
    it 'creates a GitHub repo with the right properties' do
      ...
      expect(Pivot::GitHub::Repo.from_octokit_repo(raw_repo))
        .to match_repo(repo)
    end
  end
  ...
  describe '.get_all' do
    ...
    it 'gets repositories from Octokit' do
      expect_any_instance_of(Octokit::Client)
        .to receive(:repositories).and_return(raw_repos)
    ...
    it 'calls .from_octokit_repo for every raw repository' do
      ...
      expect(Pivot::GitHub::Repo)
        .to receive(:from_octokit_repo).exactly(raw_repos.length).times
